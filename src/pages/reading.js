...
export default class ReadingPage extends React.Component {
  ...
  render() {
    return (
      <Layout location={this.props.location} title={'michaelhelvey.dev'}>
        <SEO title={'Reading'} />
        <div className='md:pt-12 pt-6 font-sans container mx-auto px-4 flex flex-col items-start'>
          ...
          <ul className='px-2 my-4 font-sans w-full'>
            {this.state.loading ? (
              ...
              this.state.books
                ...
                .map(book => (
                  <li
                    key={book.id}
                    className='mb-4 pb-2 border-b border-gray-300 bg-white w-full'
                  >
                    ...
                    <div className='text-gray-900 my-2 max-w-xl'>
                      {book.notes}
                    </div>
                  </li>
                ))
